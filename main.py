...
pygame.display.set_caption('Breakout') #give it titile
...
class wall():
  def __init__(self):
    self.width = screen_width // cols  #// is the way to don't get any decimal points
    self.height = 50
  ...
  def draw_wall(self):
    for row in self.blocks:
      for block in row:
        ...
        pygame.draw.rect(screen, bgc, (block[0]), 2)
...
class paddle():
  ...
  def reset(self):
      ...
      self.x = int((screen_width / 2) - (self.width / 2 )) #in my middle
...
class game_ball():
    ...
    def reset(self, x, y):
        self.ball_rad = 10 #radius
        self.x = x - self.ball_rad #centralize the ball
        self.y = y
...
#print player instructions
  ...
  for event in pygame.event.get():  #look for a the event in pygame
  ...
  pygame.display.update() #background collor appears!
pygame.quit()